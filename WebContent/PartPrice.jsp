...
<%
	ArrayList<PartDto> list = (ArrayList<PartDto>)request.getAttribute("list");
...
<meta charset="UTF-8">
<title>피시인사이드</title>
<link rel="stylesheet" href="css/partPriceStyle.css">
	...
	<div class="content">
		<div class="section01">
			<div class="section01Inside">
				<div class="titleTxt01">실시간 부품 시세를 확인하세요!</div>
				<div class="titleTxt02">부품 시세</div>
				<div class="searchBox">
					<form action="Controller" id="frm">
						<input type="hidden" name="command" value="partprice">
						<input type="text" class="searchInput" placeholder="부품 시세" name="str">
						<a href="#" onclick="document.getElementById('frm').submit();" class ="searchBtn">검색</a>
					</form>
				</div>
			</div>
		</div>
		<div class="section02">
			<%for(PartDto p : list){%>
				<div class='section02Inside'>
					<div class='name'><span id='partName'><%=p.getPartName()%></span></div>
					<div class='price'><span id='partPrice'><%=p.getPrice()%></span></div>
					<div class='image'><img src='PartImage/<%=p.getImage()%>.jpg'></div>
				</div>
			<%}%>
