...
<%
	...
	int recentAll = (int)request.getAttribute("recentAll");
	ArrayList<RecentDto> list = (ArrayList<RecentDto>)request.getAttribute("list");
...
<meta charset="UTF-8">
<title>피시인사이드</title>
<link rel="stylesheet" href="css/recentStyle.css">
<script src="js/jquery-3.6.0.min.js"></script>
<script src="js/recentScript.js"></script>
	<script>
	function popOpen(){
...
<body>
	...
	<div class="content">
		<div class="title">
			<span class="titleTxt01">최근거래</span>
			<span class="pageLocate">
				...
				<a href="Controller?command=recent&pageNum=1" class="page">최근거래</a>
			</span>
			<div style="margin-bottom: 20px;"></div>
			<div class="titleTxt02">최근에 고객들의 선택을 받은 상품들</div>
		</div>
		<div class="innerContent">
			...
			<div class="cntBox">
				<span>총</span>
				<span class="cntBoxBlue"><%=recentAll %></span>
				<span>건</span>
			</div>
			...
			<ul class="pcList">
			<% 
				for( RecentDto r : list){
			%>
				<li>
					<div class="pcDate">
						<span><%=r.getSELL_TIME() %></span>
					</div>
					<div class="pcName">
						<span><%=r.getCPU_MAKER()+" "+r.getCPU_TYPE()+ " " +r.getCPU_NAME()+ " " +r.getGRAPHIC_NAME() %></span>
					</div>
					<div class="pcPrice">
						<div><%=r.getPRICE()+"원" %></div>
					</div>
				</li>
				<%
				}
			...
			<div class="paging">
				<a href="Controller?command=recent&pageNum=1" class="first"></a>
				<%	
					int endPageNum = recentAll / 10 + (recentAll % 10 == 0 ? 0 : 1);
					...
					for(int i=startPageNum; i<=startPageNum+4; i++) {
						if(i!=pageNum) {
							out.println("<a href='Controller?command=recent&pageNum="+i+"' class='num'>"+i+"</a>");
						} else {
							out.println("<a href='Controller?command=recent&pageNum="+i+"' class='num now'>"+i+"</a>");
						}
				...
				%>
				<a href="Controller?command=recent&pageNum=<%=endPageNum%>" class="last"></a>
			</div>
		</div>
