...
<%
	...
	int freeBoardAll = (int)(request.getAttribute("freeBoardAll"));
	int pageNum = (int)(request.getAttribute("pageNum"));
%>
<meta charset="UTF-8">
<title>피시인사이드</title>
<link rel="stylesheet" href="css/freeBoardStyle.css">
...
<body>
	...
	<div class="title">
		<span class="titleTxt01">자유게시판</span>
	...
	<div class="innerContent">
		<div class="board_list_wrap">
			<table class="board_list">
				<tbody>
					<% for(FreeBoardDto f : list){%>
						<tr>
							<td style='color:#888888'><%=f.getBno()%></td>
							<td class='tit'><a href="Controller?command=freeboardshow&bno=<%=f.getBno()%>" style='font-weight:550;'><%=f.getTitle()%></a></td>
							<td style='color:#888888'><%=f.getTime()%></td>
							<td class='writer' style='color:#808080;'><%=f.getWriter()%></td>
						</tr>
					<%} %>	
				</tbody>
			</table>
			<div class="paging">
					<a href="Controller?command=freeboard&pageNum=1" class="first"></a>
					<%	
						int endPageNum = freeBoardAll / 10 + (freeBoardAll % 10 == 0 ? 0 : 1);
						...
						for(int i=startPageNum; i<=startPageNum+4; i++) {
							if(i!=pageNum) {
								out.println("<a href='Controller?command=freeboard&pageNum="+i+"' class='num'>"+i+"</a>");
							} else {
								out.println("<a href='Controller?command=freeboard&pageNum="+i+"' class='num now'>"+i+"</a>");
							}
					...
					%>
				<a href="Controller?command=freeboard&pageNum=<%=endPageNum%>" class="last"></a>
			</div>
			<%if(login){ %>
				<a href="Controller?command=freeboardwrite&pageNum.jsp" class="writeBtn">글쓰기</a>	
			<%} %>
		</div>
	</div>
